...
from ICCRegularizer import ICCRegularizer
...
def train(previous_ckpt_path, writer):
	...
	for epoch in range(start_epoch,start_epoch+hp.train.ckpt_interval):
		...
		for mels in dataloader:
			...
			# based on untrained model's loss value, scaled the ge2e loss value to 1.0 (maximum loss value)
			contr_loss = 0.5 * ge2e_loss(embs)
			icc_regularizer = ICCRegularizer(embs)
			loss = 1.0 * contr_loss + 0.25 * icc_regularizer
			loss.backward()
